...
export class PointResolver {
    ...
    async getPoints(@Arg("entrantId") entrantId: string): Promise<Point | null> {
        try {
            const point = await PointModel.findOne({ entrantId: new Types.ObjectId(entrantId) });
            if (!point) return null;
            
            const entrant = await EntrantModel.findById(point.entrantId).populate('associatedDog').lean();
    ...
    @Query(() => [Point])
    async getPointsByEventId(@Arg("eventId") eventId: string): Promise<Point[]> {
        try {
            console.log(`=== getPointsByEventId called with eventId: ${eventId} ===`);
            
            // First, let's find all entrants for this event
            const entrants = await EntrantModel.find({ 
                $or: [
                    { eventId: eventId },
            ...
            // Find points for these entrants
            const points = await PointModel.find({ 
            ...
            // Create a map of entrants for quick lookup
            const entrantMap = new Map();
            entrants.forEach(entrant => {
                entrantMap.set(entrant._id.toString(), entrant);
            });
            
            // Format the results with error handling
            const formattedResult: Point[] = [];
    ...
    @Mutation(() => SubmitPointsResponse)
    async submitPoints(
        @Arg("points", () => [PointsInput]) points: PointsInput[]
    ): Promise<SubmitPointsResponse> {
        try {
            ...
            // One-time cleanup of existing duplicate heatsData (only run if needed)
            // Comment out this line after running once to clean up existing corrupted data
            await this.cleanupDuplicateHeatsData();

            // Validate each point entry
            const validatedPoints: PointsInput[] = [];
            const errors: string[] = [];
            ...
            for (let i = 0; i < points.length; i++) {
                ...
                try {
                    const entrantExists = await EntrantModel.findById(point.entrantId);
                    if (!entrantExists) {
            ...
            const existingPoints = await PointModel.find({ 
                entrantId: { $in: entrantIds } 
            });
            ...
            if (existingPoints.length > 0) {
                ...
                // Remove duplicates or update existing ones
                await PointModel.deleteMany({ 
                    entrantId: { $in: duplicateIds } 
                });
            }

            // Prepare data for insertion - prioritize submitted heatsData over existing
            const pointsToCreate = await Promise.all(validatedPoints.map(async (point) => {
                // Prefer submitted heatsData over existing entrant data to allow updates
                let heatsData = point.heatsData || [];
                
                // If no heatsData submitted, fall back to existing entrant data
                if (!heatsData || heatsData.length === 0) {
                    const entrant = await EntrantModel.findById(point.entrantId);
                    heatsData = entrant?.heatsData || [];
                }
                
                // Deduplicate heatsData on server side to prevent storage of duplicates
                ...
                return {
                    entrantId: new Types.ObjectId(point.entrantId),
                    points: point.points,
                    cutoffTime: point.cutoffTime || null,
                    dogPoints: point.dogPoints || [],
            ...
            // Insert points data
            const createdPoints = await PointModel.insertMany(pointsToCreate);
            console.log(`Successfully created ${createdPoints.length} point entries`);

            // Update entrants with points data
            const entrantUpdatePromises = validatedPoints.map(async (point) => {
                try {
                    ...
                    if (point.heatsData && point.heatsData.length > 0) {
                        // Deduplicate heatsData before updating entrant
                        const uniqueHeats = new Map();
                        point.heatsData.forEach((heat: any) => {
                            const key = `${heat.heat}-${heat.temperature}-${heat.distance}-${heat.class}`;
                            uniqueHeats.set(key, heat);
                        });
                    ...
                    return await EntrantModel.findByIdAndUpdate(
                        point.entrantId,
                        { $set: updateData },
                        { new: true }
                    );
    ...
    private async cleanupDuplicateHeatsData(): Promise<void> {
        try {
            console.log("Starting cleanup of duplicate heatsData...");
            
            // Clean up duplicate heatsData in Point collection
            const points = await PointModel.find({ "heatsData.0": { $exists: true } }).lean();
            const pointUpdates = [];
            ...
            if (pointUpdates.length > 0) {
                await PointModel.bulkWrite(pointUpdates);
                console.log(`Cleaned up duplicate heatsData in ${pointUpdates.length} point records`);
            }
            
            // Clean up duplicate heatsData in Entrant collection
            ...
            for (const entrant of entrants) {
                if (entrant.heatsData && entrant.heatsData.length > 0) {
                    const uniqueHeats = new Map();
                    entrant.heatsData.forEach((heat: any) => {
                        const key = `${heat.heat}-${heat.temperature}-${heat.distance}-${heat.class}`;
                        uniqueHeats.set(key, heat);
                    });
                    const deduplicatedHeatsData = Array.from(uniqueHeats.values());
            ...
            // Don't throw error - cleanup failure shouldn't block point submission
        }
    }
} 