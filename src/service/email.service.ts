...
export class EmailService {
  ...
  constructor() {
    ...
    if (this.config) {
      ...
      // Don't await here, but log if it fails
  ...
  private validateAndGetConfig(): EmailConfig | null {
    ...
    if (missingVars.length > 0) {
      logger.warn(
        'Email service configuration is incomplete. Missing environment variables:',
        missingVars.join(', ')
      );
      return null;
    }

    return requiredEnvVars as EmailConfig;
  }
  ...
  async sendFormNotification(clubEmail: string, formDetails: FormDetails): Promise<void> {
    if (!clubEmail) {
      logger.warn('No club email provided. Skipping form notification email.');
      return;
    }
    ...
    try {
      ...
      logger.info(`Musher registration notification email sent to ${clubEmail}`);
    } catch (error) {
      logger.error('Error sending musher registration notification email:', error);
      throw error;
    }
  }

  async sendGenericNotification(to: string, subject: string, message: string): Promise<void> {
    if (!to) return;
    try {
      const transporter = await this.ensureTransporter();
      await transporter.sendMail({
      ...
      logger.error('Error sending generic notification email:', error);
    }
  }
} 